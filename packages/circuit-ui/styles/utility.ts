/**
 * Copyright 2023, SumUp Ltd.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

import _classes from './utility.module.css';

// This explicit remapping is needed so TypeScript infers the types correctly
export const utilClasses = {
  center: _classes.center,
  hideVisually: _classes['hide-visually'],
  hideScrollbar: _classes['hide-scrollbar'],
  focusVisible: _classes['focus-visible'],
  focusVisibleInset: _classes['focus-visible-inset'],
  // all directions
  marginBit: _classes['margin-bit'],
  marginByte: _classes['margin-byte'],
  marginKilo: _classes['margin-kilo'],
  marginMega: _classes['margin-mega'],
  marginGiga: _classes['margin-giga'],
  marginTera: _classes['margin-tera'],
  marginPeta: _classes['margin-peta'],
  marginExa: _classes['margin-exa'],
  marginZetta: _classes['margin-zetta'],
  // left
  marginLeftBit: _classes['margin-left-bit'],
  marginLeftByte: _classes['margin-left-byte'],
  marginLeftKilo: _classes['margin-left-kilo'],
  marginLeftMega: _classes['margin-left-mega'],
  marginLeftGiga: _classes['margin-left-giga'],
  marginLeftTera: _classes['margin-left-tera'],
  marginLeftPeta: _classes['margin-left-peta'],
  marginLeftExa: _classes['margin-left-exa'],
  marginLeftZetta: _classes['margin-left-zetta'],
  // right
  marginRightBit: _classes['margin-right-bit'],
  marginRightByte: _classes['margin-right-byte'],
  marginRightKilo: _classes['margin-right-kilo'],
  marginRightMega: _classes['margin-right-mega'],
  marginRightGiga: _classes['margin-right-giga'],
  marginRightTera: _classes['margin-right-tera'],
  marginRightPeta: _classes['margin-right-peta'],
  marginRightExa: _classes['margin-right-exa'],
  marginRightZetta: _classes['margin-right-zetta'],
  // top
  marginTopBit: _classes['margin-top-bit'],
  marginTopByte: _classes['margin-top-byte'],
  marginTopKilo: _classes['margin-top-kilo'],
  marginTopMega: _classes['margin-top-mega'],
  marginTopGiga: _classes['margin-top-giga'],
  marginTopTera: _classes['margin-top-tera'],
  marginTopPeta: _classes['margin-top-peta'],
  marginTopExa: _classes['margin-top-exa'],
  marginTopZetta: _classes['margin-top-zetta'],
  // bottom
  marginBottomBit: _classes['margin-bottom-bit'],
  marginBottomByte: _classes['margin-bottom-byte'],
  marginBottomKilo: _classes['margin-bottom-kilo'],
  marginBottomMega: _classes['margin-bottom-mega'],
  marginBottomGiga: _classes['margin-bottom-giga'],
  marginBottomTera: _classes['margin-bottom-tera'],
  marginBottomPeta: _classes['margin-bottom-peta'],
  marginBottomExa: _classes['margin-bottom-exa'],
  marginBottomZetta: _classes['margin-bottom-zetta'],
};
