...
The component allows the user to search and select their country code, and normalizes the input to the [E.164 format](https://www.itu.int/rec/t-rec-e.164/en).
...
Pass pre-computed labels via `countryCode.options[].label` or `countryCode.getOptionLabel` to avoid SSR/client mismatches from `Intl.DisplayNames` in server-rendered apps.
