...
- To enable multiple selections, set the `selectionMode` prop to "multiple". In this mode, the `value` prop must be an array of `AutocompleteInputOption` objects. The `onChange` callback will still receive a single AutocompleteInputOption—the option that was just selected or removed.
