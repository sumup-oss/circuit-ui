...
### Image
The NotificationModal component provides two ways for adding an image through the `image` prop object:
- `illustration`: use one of the available illustrations from the [Illustration](Features/Illustration/Docs) library.
```tsx
<NotificationModal {...props} image={{ illustration: 'celebration'}}/>
```
- `src`: A URL to an image asset.
```tsx
<NotificationModal {...props} image={{ src: 'https://example.com/image.png'}}/>
```
