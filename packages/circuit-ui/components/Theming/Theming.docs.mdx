...
At the core of Circuit UI theming are semantic color tokens. Instead of referencing color values (as hex codes, for example), Circuit UI components reference semantic color names, which are mapped to a color value depending on te theme.
