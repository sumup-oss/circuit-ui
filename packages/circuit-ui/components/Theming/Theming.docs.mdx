...
```ts
type ColorRole = 'accent' | 'confirm' | 'notify' | 'alert'; // Q: and promo? // Q: notify doesn't work with background.default in the current theme
...
type BackgroundRoles = {
  [key in ColorRole]: {
    ...
    subtle?: ColorWithState; // Q: currently only used in accent role (NotificationBanner), should we add it for others?
  };
};
...
- Role colors as backgrounds typically have a single variant in our UIs, except `background.accent`, which has a subtle variant in the `NotificationBanner`. Do we need to add subtle variants for all role colors? I suggest that subtle role backgrounds need to have 4.5:1 contrast with the default foreground color, to avoid the need for e.g. `onAccentSubtle`
