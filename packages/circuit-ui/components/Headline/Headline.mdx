...
You can also add visually hidden headings using the `hideVisually` utility class from `utilClasses`:
...
If you need to render large text that doesn't describe a section, don't use a semantic heading element, and instead use CSS to enlarge the text, or use the [Numeral](Numeral) component for a more accessible way to display numbers.
