...
You can also add visually hidden headings using the `hideVisually` utility class from [`utilClasses`](Features/Utility-Classes/spacing):
...
function Product() {
  return (
    <section>
      ...
      <Body style={description}>A description of my product.</Body>
    </section>
  );
}
}
...
If you need to render large text to display numbers, use the [Numeral](Numeral) component for better accessibility:
