...
### inline
Place your dialog content directly in the `Dialog` component:

```tsx
import { Dialog, Body, Button } from '@sumup-oss/circuit-ui';
import {Heading} from '@storybook/blocks';

return (
  <Dialog open={true} closeButtonLabel="Close" aria-labelledby="dialog-title">
    {() => (
        <>
        <Heading as="h2" id="dialog-title">Dialog title</Heading>
