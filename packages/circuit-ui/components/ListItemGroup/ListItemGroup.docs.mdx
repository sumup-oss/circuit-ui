...
- **Do** use size `two` Body for the group `label` but render it as a heading for accessibility (e.g. `as="h4"`) when using a custom component.
- **Do** use size `two` Body for the group `details` when using a custom component.
