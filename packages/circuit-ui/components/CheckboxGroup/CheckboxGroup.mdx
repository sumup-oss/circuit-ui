...
- **Don not** rely on tooltips to explain checkboxes.
...
The `label` prop passed to the CheckboxGroup or one of its `options` constitutes the input's label. This label should be clear and concise, and should not contain any formatted or semantic content (such as headings or lists).
