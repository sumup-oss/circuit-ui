...
If you wish to have more control on your side panels, you can also use the `SidePanel` component inline in your component tree.

```tsx
import { SidePanel, Button, Body } from "@sumup-oss/circuit-ui";
  ...
  return (
    <>
      ...
      <SidePanel
        ...
        headline="More information"
      >
        <Body>Some additional information</Body>
      </SidePanel>
    </>
  );
}
...
The `useSidePanel` hook handles the stacking of side panels for you. If you wish to use the SidePanel inline and handle stacking yourself, use the `onBack` prop to a stacked `SidePanel` component to show a back button that closes the topmost panel.
