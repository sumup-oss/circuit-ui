...
Use the component when displaying a date time to users. The date must be a `Temporal.ZonedDateTime` object or a string in the [ISO-8601](https://en.wikipedia.org/wiki/ISO_8601) format including an [IANA time zone identifier](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones).
