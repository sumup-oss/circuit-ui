...
`TabList` and `TabPanel` can be used independently, depending on how much control you need, there are two approaches.

#### Simplified with the `tabs` prop

Pass your tabs as a `tabs` array to `TabList`. Selection state, keyboard navigation, and ARIA attributes are managed internally. Use the `onTabChange` callback to track which panel to show.
...
- For `TabPanel`, assign a unique `id` following the `panel-{id}` convention and set `aria-labelledby` to `tab-{id}`.

<Story of={Stories.WithTabsProp} />

#### Flexible with `Tab` children
...
- Set the `as` prop of `Tab` to `"listitem"` to wrap your tab in an element with role `listitem`.
...
Alternatively, use the `tabs` prop and include an `href` on each item for a simpler setup:

<Story of={TabListStories.NavigationWithTabsProp} />
