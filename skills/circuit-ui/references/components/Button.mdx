...
Pass an `href` prop to render the Button as a link. That changes the underlying element from a `button` to an `a` (or to whatever Link component is provided via [ComponentsContext](Features/Base-Components/Docs).
For external links (with `rel="external"` or `target="_blank"`), also pass an `externalLabel` prop for screen readers to announce them correctly.
