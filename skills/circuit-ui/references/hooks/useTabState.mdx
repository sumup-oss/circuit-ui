...
`TabList` and `TabPanel` can be used independently, depending on how much control you need, you can opt for one of the following approaches:

#### Simplified with the `tabs` prop

Pass your tabs as a `tabs` array to `TabList`. Selection state, keyboard navigation, and ARIA attributes are managed internally.
