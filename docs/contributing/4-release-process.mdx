...
- **`canary`** - This is a branch you can use to publish a prerelease version if you need to deploy the changes somewhere to test them. `canary` is a throw-away branch that can be recreated from `main` at any time. **Hint**: If you only need to test your changes locally, you can use `npm run add ./path-to-circuit-ui` to link the development version.
...
```bash
npm run add @sumup/circuit-ui@<release-channel>
```
