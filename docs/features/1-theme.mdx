...
Circuit UI's [ESLint plugin](Packages/eslint-plugin-circuit-ui/Docs)  includes rules (`no-invalid-custom-properties` and `no-deprecated-custom-properties`) to ensure proper usage of the design tokens.
