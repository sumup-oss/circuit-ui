...
The theme used throughout Circuit UI comes from SumUp's design token library, [`@sumup/design-tokens`](https://github.com/sumup-oss/circuit-ui/tree/main/packages/design-tokens), which is a required peer dependency of `@sumup/circuit-ui`. Refer to its documentation for usage instructions.
