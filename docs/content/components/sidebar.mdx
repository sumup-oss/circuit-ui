...
import SidebarWithState from './sidebar';

# Sidebar

<Status.InReview/> <Status.Description size={Text.KILO}>Under active development</Status.Description>
...
and the second level of navigations, keeping a strong anchor for our merchants to always know where they are.
...
The sidebar supports first and second level items. The first level is used for primary actions/value for our merchants within a product.
...
- **Do** add icons to first level items to help our merchants understand what they should expect in that section
