...
Once you run the Chromatic build on a branch the baseline will be established. 
This will capture snapshot of each story and sets it as test baseline. Every next build will generate new snapshots and compare them against the baseline and detect the UI changes.
