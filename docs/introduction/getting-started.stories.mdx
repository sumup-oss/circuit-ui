...
For Circuit UI to render as expected on iOS devices, make sure that the viewport meta tag is set to `cover`, and that your app has padding for iOS safe area insets:
...
This ensures that the UI covers the entire screen, while Circuit UI modals and navigation are still rendered within iOS safe areas.
