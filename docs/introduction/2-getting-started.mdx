...
In order to use illustrations from the `@sumup-oss/illustrations` package in Circuit UI components such as the NotificationBanner, you need to manually install the package and import its styles:

1. Install the package:
```sh
npm install @sumup-oss/illustrations
```
2. Import the illustration styles:
```tsx
// /app/layout.tsx or /pages/_app.tsx for Next.js
...
Make sure that the viewport meta tag includes `viewport-fit=cover`, and that your app has the right padding to render inside [CSS safe areas](<https://developer.mozilla.org/en-US/docs/Web/CSS/env()>):

```tsx
// /app/layout.tsx or /pages/_document.tsx for Next.js
...
Wrap your application in the `I18nProvider` to configure the locale that the design system components should use for [internationalization](Introduction/Internationalization/Docs). Optionally add the relevant context providers if you're planning on using any [modal](Components/Modal/Docs) or [toast](Notification/NotificationToast/Docs) components.

```tsx
// /app/layout.tsx or /pages/_app.tsx for Next.js
import { I18nProvider, ToastProvider, ModalProvider } from '@sumup-oss/circuit-ui';
